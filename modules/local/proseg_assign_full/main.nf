...
// Stage 2 (full-scale) variant. Runs in the xenium-seg-search container because
// the proseg container has no Python.
process PROSEG_ASSIGN_FULL {
    tag "${meta.id}"
    label 'process_low'
    ...
    input:
    tuple val(meta), path(transcripts), path(transcript_metadata), path(cell_polygons)
